...
- 자기 참조 변수
- 함수가 어떻게 호출되냐에 따라 동적으로 결정
  - 전역에서 일반함수로 호출 → 전역 객체
  - 객체의 메소드로 호출 → 호출한 인스턴스
  - 이벤트 핸들러 → 이벤트가 발생한 DOM
  - 명시적으로 특정 객체를 this에 연동하고 싶다면 call, apply 같은 메소드 사용
...
- 스레드
  ...
  - 멀티스레드 지원: Node.js Worker, 브라우저 WebWorker
- 동기/비동기
  - 작업 순서
